...
import { IAuthBuyerMessageDetails, IAuthDocument, firstLetterUppercase, lowerCase, winstonLogger } from '@mayank30041995/jobber-shared';
...
export async function getAuthUserByOTP(otp: string): Promise<IAuthDocument | undefined> {
  try {
    const user: Model = (await AuthModel.findOne({
      where: {
        [Op.and]: [{ otp } as any, { otpExpiration: { [Op.gt]: new Date() } }]
      }
    })) as Model;
    return user?.dataValues;
  } catch (error) {
