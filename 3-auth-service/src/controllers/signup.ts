...
import {
  ...
  lowerCase,
  uploads
} from '@mayank30041995/jobber-shared';
...
export async function create(req: Request, res: Response): Promise<void> {
  ...
  console.log('PROFILE PICTURE PREFIX:', typeof profilePicture === 'string' ? profilePicture.substring(0, 30) : profilePicture);
  console.log('PROFILE PICTURE LENGTH:', typeof profilePicture === 'string' ? profilePicture.length : 0);
  ...
  const authData: IAuthDocument = {
    ...
    browserName,
    deviceType
  } as unknown as IAuthDocument; // unknown added for now will remove later after @mayank30041995/jobber-shared is updated
